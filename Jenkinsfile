    ...
    stage('image build and push') {
      steps {
        container(name: 'kaniko') {
          sh '/kaniko/executor -f `pwd`/Dockerfile -c `pwd`/src --cache=true           --destination=cloudnative.azurecr.io/httpserver:${DATED_GIT_HASH}                   --insecure                   --skip-tls-verify                    -v=debug'
        }
